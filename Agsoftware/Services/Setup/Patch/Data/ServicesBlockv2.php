<?php

namespace Agsoftware\Services\Setup\Patch\Data;

class ServicesBlockv2 implements \Magento\Framework\Setup\Patch\DataPatchInterface
{
    /**
     * CreateHeaderBlock constructor.
     * @param ModuleDataSetupInterface $moduleDataSetup
     * @param \Magento\Cms\Model\BlockRepository $blockRepository
     * @param \Magento\Cms\Api\Data\BlockInterface $block
     */

    public function __construct(
        \Magento\Framework\Setup\ModuleDataSetupInterface $moduleDataSetup,
        \Magento\Cms\Model\BlockRepository $blockRepository,
        \Magento\Cms\Api\Data\BlockInterfaceFactory $block,
        \Magento\Cms\Api\GetBlockByIdentifierInterface $blockByIdentifier
    ) {
        $this->blockRepository = $blockRepository;
        $this->block = $block;
        $this->moduleDataSetup = $moduleDataSetup;
        $this->blockByIdentifier = $blockByIdentifier;
    }
    /**
     * {@inheritdoc}
     */
    public function apply()
    {
        $this->moduleDataSetup->getConnection()->startSetup();

        $block_data_head= [
            'title' => 'block1',
            'identifier' => 'servicesb',
            'is_active' => 1,
            'content' => file_get_contents(__DIR__ . '/html/ServicesBlock1.html'),
        ];
        $this->makeBackup($block_data_head);
        $block_head = $this->block->create();
        $block_head->addData($block_data_head);
        $block_head->setStores([0]);
        $this->blockRepository->save($block_head);
        $this->moduleDataSetup->getConnection()->endSetup();
    
    
    $block_data_head= [
            'title' => 'Bannerservicies',
            'identifier' => 'servicesa',
            'is_active' => 1,
            'content' => file_get_contents(__DIR__ . '/html/ServicesBanner.html'),
        ];

        $this->makeBackup($block_data_head);
        $block_head = $this->block->create();
        $block_head->addData($block_data_head);
        $block_head->setStores([0]);
        $this->blockRepository->save($block_head);
        $this->moduleDataSetup->getConnection()->endSetup();
    
        
         $block_data_head= [
            'title' => 'block2',
            'identifier' => 'servicesc',
            'is_active' => 1,
            'content' => file_get_contents(__DIR__ . '/html/ServicesBlock2.html'),
        ];
        $this->makeBackup($block_data_head);
        $block_head = $this->block->create();
        $block_head->addData($block_data_head);
        $block_head->setStores([0]);
        $this->blockRepository->save($block_head);
        $this->moduleDataSetup->getConnection()->endSetup();
        
        
        
        
         $block_data_head= [
            'title' => 'Banner24',
            'identifier' => 'bloquesitobanner',
            'is_active' => 1,
            'content' => file_get_contents(__DIR__ . '/html/ServicesBlock24hr.html'),
        ];
        $this->makeBackup($block_data_head);
        $block_head = $this->block->create();
        $block_head->addData($block_data_head);
        $block_head->setStores([0]);
        $this->blockRepository->save($block_head);
        $this->moduleDataSetup->getConnection()->endSetup();
        
        
        $block_data_head= [
            'title' => 'block3',
            'identifier' => 'servicesd',
            'is_active' => 1,
            'content' => file_get_contents(__DIR__ . '/html/ServicesBlock3.html'),
        ];
        $this->makeBackup($block_data_head);
        $block_head = $this->block->create();
        $block_head->addData($block_data_head);
        $block_head->setStores([0]);
        $this->blockRepository->save($block_head);
        $this->moduleDataSetup->getConnection()->endSetup();
        
        
        
        
        
         $block_data_head= [
            'title' => 'block4',
            'identifier' => 'servicesf',
            'is_active' => 1,
            'content' => file_get_contents(__DIR__ . '/html/ServicesBlock4.html'),
        ];
        $this->makeBackup($block_data_head);
        $block_head = $this->block->create();
        $block_head->addData($block_data_head);
        $block_head->setStores([0]);
        $this->blockRepository->save($block_head);
        $this->moduleDataSetup->getConnection()->endSetup();
        
    }

    public function makeBackup($data){
        $block = $this->block->create()->load($data['identifier'],'identifier');
        if($block->getId()>0){
            $backup = $this->block->create()->load($data['identifier'].'-backup','identifier');
            if($backup->getId()>0){
                $backup->delete();
            }
            $block->setIdentifier($data['identifier'].'-backup')->setActive(0)->save();
        }
    }


      /**
     * @inheritdoc
     */
    public static function getDependencies()
    {
        return [];
    }
    /**
     * @inheritdoc
     */
    public function getAliases()
    {
        return [];
    }
    /**
     * Revert patch
     */
    public function revert()
    {
        $this->moduleDataSetup->getConnection()->startSetup();
        //code
        $this->moduleDataSetup->getConnection()->endSetup();
    }
}
